...
btn.addEventListener('click', (e) => {
  // here this part is more important
  ...
  publishYear.textContent = year.value;

  bookSection.appendChild(bookTitle);
  bookSection.appendChild(bookAuthor);
  bookSection.appendChild(publishYear);
...
// function to fetch use data from the random user API
async function fetchUser(query) {
  if (!query) return; // skip empty queries
  try {
    const response = await fetch('https://randomuser.me/api/?result=1'); // result=1 for fetching only one record at a time
...
// function to display use data in the user card
function displayUser(user) {
  const userCard = document.querySelector('.user-card');
  userCard.innerHTML = `<img src="${user.picture.large}" alt="User Picture">
  <h2>${user.name.first} ${user.name.last}</h2>
  <p>${user.email}</p>`;

  userCard.style.dispplay = 'block';
}

// Debounced version of the fetchUser function
